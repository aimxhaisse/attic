...
timestamp-inactive.  If it is this symbol, the exporter uses the first
inactive timestamp in an entry headline."
  :type '(choice
	  (string :tag "Headline property" :value "atom_published")
	  (const :tag "First inactive timestamp" timestamp-inactive))
  :group 'org-export-atom)
...
PROJECT and publishes them as one single atom feed."
  (let* ((project-plist (cdr project))
	 ...
	 (sitemap-title (or (plist-get project-plist :feed-title)
			    (plist-get project-plist :sitemap-title)
	...
	(dtime (or (org-entry-get
		   ...
		   (org-entry-get
		    nil (org-atom-expand-special-timestamp-property
			 org-atom-updated-property-name)))))
    (unless dtime
      (if (eq org-atom-published-property-name 'timestamp_inactive)
...
(defun org-atom-xml-element (element)
  "Return XML fragment of ELEMENT.
ELEMENT is a list of the following format:

 (TAG ATTRIBUTES CHILDNODES)

Where TAG is the symbol or string of the XML tag, ATTRIBUTES is a
list of cons of attributes and CHILDNODES is a either a list of
zero or more ELEMENTs or a string with the element content."
  (format "<%s %s%s>"
	  (car element)
	  (mapconcat 'org-atom-xml-attribute (cadr element) " ")
	  (if (null (caddr element))
	      "/"
	    (format ">%s</%s"
		    (if (stringp (caddr element))
			(xml-escape-string (caddr element))
		      (mapconcat 'org-atom-xml-element (caddr element) ""))
		    (car element)))))

(defun org-atom-xml-attribute (attr)
  "Return XML attribute string.
ATTR is a cons with attribute name in car and attribute value in
cdr."
